...
### App Deployment
To deploy the app you need to run the following command from inside the `alex` directory:
...
From inside the `alex` directory run:
...
`pytest tests/` or `pytest -m api` from inside the `alex` directory on your command line to run the tests.
